    ...
    return (
        <div className="about-page">
            ...
            <div className="about-text">
                <h3>About</h3>
                <p>TransitVis is a prototype open source tool built to visualize real-time bus location data from the OneBusAway API. This tool was built as part of an ongoing study of bus interference at the University of Washington. On the backend, bus location data is scraped from the OneBusAway API, and uploaded to an AWS database. Once daily at 1AM PST, updated performance metrics are calculated from this data and summarized in this online tool. Transit speeds are calculated at 30 second intervals for each active trip and assigned to the closest transit segment. Roadway segments from the Seattle Streets dataset are stored using a spatial index to facilitate quick nearest-neighbor matching, and only segments that are traversed by a particular route are able to have tracked data from that route assigned to them. In cases where a segment does not have any tracked data available for a given day, the most recently available data is assigned.</p>
